...
int ToggleButton::calculateEndPosition() const {
    ...
    return m_toggled ? width() - thumbRadius() * 2 - padding() - borderWidth/2 : padding() + borderWidth/2;
}
...
void ToggleButton::startAnimations() {
    ...
    m_colorAnimation->setEndValue(m_toggled ? 1.0 : 0.0);
...
void ToggleButton::updateAppearance() {
    ...
    setAnimationProgress(m_toggled ? 1.0 : 0.0);
}
...
void ToggleButton::setToggled(bool t) {
    if (m_toggled == t) return;

    m_toggled = t;
    emit QPushButton::toggled(m_toggled);
    startAnimations();
}
